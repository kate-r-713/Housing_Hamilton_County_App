...
neighborhood = st.selectbox(
    "Neighborhood Code",
    sorted(model_data['NEIGHBORHOOD_CODE'].astype(str).unique())
)

district = st.selectbox(
    "District",
    sorted(model_data['DISTRICT'].astype(str).unique())
)

property_type = st.selectbox(
    "Property Type Code",
    sorted(model_data['PROPERTY_TYPE_CODE'].astype(str).unique())
)

current_use = st.selectbox(
    "Current Use Code",
    sorted(model_data['CURRENT_USE_CODE'].astype(str).unique())
)

zoning = st.selectbox(
    "Zoning",
    sorted(model_data['ZONING'].astype(str).unique())
)
